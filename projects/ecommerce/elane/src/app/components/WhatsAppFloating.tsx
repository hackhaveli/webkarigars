...
export default function WhatsAppFloating() {
  ...
  useEffect(() => {
    supabase.from('site_settings').select('value').eq('key', 'whatsapp_number').single()
      .then(({ data }) => { if (data) setPhone((data as any).value || '1234567890'); });
  }, []);
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      <button onClick={openWhatsApp} aria-label="Chat on WhatsApp"
        className="wa-pulse glow-accent w-14 h-14 bg-[#25D366] text-white flex items-center justify-center shadow-lg hover:scale-110 transition-transform duration-300 rounded-full"
